...
class ABCIWakefield(Geometry):
    ...
    def run(self, no_of_cells, no_of_modules, mid_cells_par, l_end_cell_par, r_end_cell_par,
        ...
        # UBT = 0.3  # The last longitudinal coordinate relative to the head of the beam, up to which the potentials
        # are calculated (defaults 10*Sig). The longer the better resolution of impedance
        # bunch_length = bunch_length
        sig_var = [x * 1e-3 for x in [bunch_length]]  # bunch length converted to m

        # not needed for our parametrization
        end_type = 1  # if _type = 1 the  HALF cell is changed for tuning. If _type = 2 the WHOLE  cell is changed for
        # tuning
        ...
        for i_out in range(1):  # check!!!
            ...
            with open(fname, 'w') as f:
                    ...
                    for i_mode in range(1, module_nu + 1):
                        ...
                        if self.WG_R > 0:
                            if end_R == 2:
                                ...
                                f.write('{} {} \n'.format(self.ri_R, self.WG_L + self.WG_R + self.L_L + self.L_R + (
                                            i_mode - 1) * self.L_all))
                ...
                if n > 1:
                    for i_mode in range(1, module_nu + 1):
                        ...
                        if self.WG_R > 0:
                            if end_R == 2:
                                f.write('{} {} \n'.format(self.Rbp_R,
                                                          self.WG_L + self.WG_R + self.L_L + self.L_R + 2 * (
                                                                      n - 1) * self.L_M + (i_mode - 1) * self.L_all))
                            else:
                                f.write('{} {} \n'.format(self.ri_R, self.WG_L + self.WG_R + self.L_L + self.L_R + 2 * (
                                            n - 1) * self.L_M + (i_mode - 1) * self.L_all))

                    f.write('0 {} \n'.format(self.WG_L + self.WG_R + self.L_L + self.L_R + 2 * (n - 1) * self.L_M + (
                                module_nu - 1) * self.L_all))
                    f.write('0 0 \n')
                    f.write('9999. 9999. \n')
                ...
                f.write(' &WAKE   &END \n')
                f.write(
                    ' &PLOT  LCAVIN = .T., LCAVUS = .T., LPLW = .T., LFFT = .T., LSPEC = .T., LINTZ = .F., LPATH = .T., NFS = {} &END \n'.format(
                        NFS))
            ...
            subprocess.call([exe_path, fr'{abci_path}\{name}\Cavity_MROT_{MROT}.abc'], stdout=subprocess.DEVNULL,
...
if __name__ == '__main__':
    ...
    no_of_cells = 9
    no_of_modules = 1
    mid_cells_parameters = [42, 42, 12, 19, 35, 57.7, 103.3]  # [A_m, B_m, a_m, b_m, Ri_m, L_m, Req_m]
    left_end_cell_parameters = [40.3, 40.3, 10, 13.5, 39, 56.0, 103.3]  # [A_e, B_e, a_e, b_e, Ri_e, L_e, Req_e]
    right_end_cell_parameters = [42, 42, 9, 12.8, 39, 57.0, 103.3]
    name = "TESLA"
    path = None  # Path to write simulation files to. If None, the current directory is taken as the path
    MROT = 0  # Polarization. 0 -  Longitudinal 1 - Transverse
    bunch_length = 25  # bunch length
    MT = 4
    NFS = 10000  # Number of frequency samples
    UBT = 50  # simulated wakelength
    ...
    abciwakefield.run(no_of_cells, no_of_modules, mid_cells_parameters, left_end_cell_parameters,
                      right_end_cell_parameters,
                      name, path, MROT=MROT, bunch_length=bunch_length, MT=MT, NFS=NFS, UBT=UBT, DDZ_SIG=DDZ_SIG,
                      DDR_SIG=DDR_SIG)
