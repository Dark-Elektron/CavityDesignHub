...
def plot_triplot_win(s = None,side = None,ss = None): 
    if len(varargin) < 1:
        s = 1
    ...
    if ok > 0:
        ...
        if ok1 * ok2 == 0:
            ...
            if ss > np.logical_and(0,side) == 'l':
                cl = error_message(np.array(['Plotting the triplot (counter, enhanced ','counter and impact energy). Warm side.']))
            else:
                if ss > np.logical_and(0,side) == 'r':
                    cl = error_message(np.array(['Plotting the triplot (counter, enhanced ','counter and impact energy). Cold side.']))
            Window = plt.figure(5)
            clf
