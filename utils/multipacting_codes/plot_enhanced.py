...
def plot_enhanced(side = None): 
    ...
    if ok1 * ok2 > 0:
        ...
        if gtype == 1:
            load_output_data
            if n == 0:
                cl = error_message(np.array(['Unable to plot the enhanced counter. No ','initial points.']))
                ok = 0
            if ok > 0:
                ...
                subplot(1,1,1)
                plt.plot(flevel / 1000.0,A)
                grid
                plt.ylabel(np.array(['e_{',num2str(N),'} ']))
                plt.xlabel('Peak Electric Field  [kV/m]')
                plt.title(np.array(['MultiPac 2.1          Enhanced Counter Function         ',date]))
                ax = axis
                plt.axis(np.array([np.amin(flevel) / 1000.0,np.amax(flevel) / 1000.0,ax(3),ax(4)]))
                htyp = uicontrol('Style','Popup','String','lin|rlin|log|rlog','Units','Normalized','Position',np.array([0.78,0.85,0.12,0.05]),'Callback','ploten','Tag','Plot type')
...
#		       'Callback','ploten','Tag','Dimension');
            ...
            if gtype == 2:
                ...
                if ok > 0:
                    ...
                    htyp = uicontrol('Style','Popup','String','lin|rlin|log|rlog','Units','Normalized','Position',np.array([0.78,0.85,0.12,0.05]),'Callback','ploten','Tag','Plot type')
                    hdim = uicontrol('Style','Popup','String','power|voltage','Units','Normalized','Position',np.array([0.78,0.78,0.12,0.05]),'Callback','ploten','Tag','Dimension')
            else:
                if gtype == 3:
                    load_output_window
                    if ok > 0:
                        ...
                        if side == 'l':
                            ...
                            if side == 'r':
                                ...
                                cl = error_message(np.array(['To plot the enhanced counter function in ','window, choose warm or cold side.']))
                                return
                        Window = plt.figure(5)
                        clf
                        set(Window,'name','MULTIPAC - Output Window I')
