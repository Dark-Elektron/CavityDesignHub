% Function program plot_inisecang(s)
% ---------------------------------------------------------------------
% Plots the initial points, the geometry, the secondary yield curves 
% and the initial emission angle distribution.
%
% ---------------------------------------------------------------------
% CALLS TO : check_fieldparam.m, check_inputs.m, check_inputs_win.m, 
%            error_message.m
% 14/05/00 : Pasi Yla-Oijala, Rolf Nevanlinna Institute
% 28/09/00 :                - plot for the initial angle added
% ---------------------------------------------------------------------

function plot_inisecang(s)
...
if s == 1
  ...
  if gtype <= 2
    ...
    if ok == 0
      error_message(['Initials file is missing. Choose Create Inputs in ' ...
		     'menu Inputs.']);
      return;
    else
      load initials_temp
...
if ok > 0
  cl = error_message(['Plotting the geometry, the secondary yield and ' ...
		      'emission angle.']);
  ...
  title('Emission angle distribution')
end
% ---------------------------------------------------------------------
