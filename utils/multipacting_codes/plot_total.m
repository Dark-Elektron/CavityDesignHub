...
if ok1*ok2 > 0
  ...
  if gtype == 1
    ...
    if ok > 0      
      ...
      subplot(1,1,1)
      plot(flevel/1e3,At),grid
      ylabel(['t_{' num2str(N) '} ' ])
      xlabel('Peak Electric Field  [kV/m]')
      title(['MultiPac 2.1           Total Counter Function         ' date ])
      ax = axis;
...
%	       'Callback','plotto','Tag','Dimension');
  ...
  elseif gtype == 3
	...
	A = Ar; n = nr;
      ...
      if n == 0
	cl=error_message(['Unable to plot the total counter. No initial ' ...
			  'points.']);
	ok = 0;
	return;
      end
      ...
      elseif side == 'r'
        ...
        cl = error_message(['To plot the total counter function in ' ...
