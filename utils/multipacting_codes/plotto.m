...
if tval == 1
  ...
  elseif dval == 3
    plot(Efl/1e3,At),grid
    xlabel('Peak Electric Field   [kV/m]')
    ax = axis;
    axis([min(Efl)/1e3,max(Efl)/1e3,ax(3),ax(4)]);
  elseif dval == 1
    plot(Pow/1e3,At),grid
    xlabel('Forward Power   [kW]')
    ax = axis;
    axis([min(Pow)/1e3,max(Pow)/1e3,ax(3),ax(4)]);
  end
  ylabel(['t_{' num2str(N) '} ' ])
  title(['MultiPac 2.0            Total Counter Function           ' date ])
elseif tval == 2
  if dval == 2
    plot(U/1e3,At/n),grid
    xlabel('Peak voltage   [kV]')
    ax = axis;
    axis([min(U)/1e3,max(U)/1e3,ax(3),ax(4)]);
  elseif dval == 3
    plot(Efl/1e3,At/n),grid
    xlabel('Peak Electric Field   [kV/m]')
    ax = axis;
    axis([min(Efl)/1e3,max(Efl)/1e3,ax(3),ax(4)]);
  elseif dval == 1
    plot(Pow/1e3,At/n),grid
    xlabel('Forward Power   [kW]')
    ax = axis;
    axis([min(Pow)/1e3,max(Pow)/1e3,ax(3),ax(4)]);
  end
  ylabel(['t_{' num2str(N) '} / c_0 ' ])
  title(['MultiPac 2.0       Relative Total Counter Function          ' date ])
elseif tval == 3  
  if dval == 2
    semilogy(U/1e3,At+1),grid  
    xlabel('Peak voltage   [kV]')
    ax = axis;
    axis([min(U)/1e3,max(U)/1e3,1,ax(4)]);
  elseif dval == 3
    semilogy(Efl/1e3,At+1),grid  
    xlabel('Peak Electric Field   [kV/m]')
    ax = axis;
    axis([min(Efl)/1e3,max(Efl)/1e3,1,ax(4)]);
  elseif dval == 1
    semilogy(Pow/1e3,At+1),grid  
    xlabel('Forward Power   [kW]')
    ax = axis;
    axis([min(Pow)/1e3,max(Pow)/1e3,1,ax(4)]);
  end
  ylabel(['t_{' num2str(N) '} ' ])
  title(['MultiPac 2.0       Total Counter Function in log_{10}-scale      ' date ])
elseif tval == 4
  if dval == 2
  ...
  ylabel(['t_{' num2str(N) '} / c_0 ' ])
  title(['MultiPac 2.0     Relative Total Counter Function in log_{10}-scale      ' date ])
end
% --------------------------------------------------------------------
