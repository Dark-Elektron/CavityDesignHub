...
from node_editor.node_socket import Socket, LEFT_BOTTOM, LEFT_CENTER, LEFT_TOP, RIGHT_BOTTOM, RIGHT_CENTER, RIGHT_TOP
...
class Node(Serializable):
    ...
    def __init__(self, scene: 'Scene', title: str="Undefined Node", inputs: list=[], outputs: list=[]):
        ...
        :type title: str
        :param inputs: list of :class:`~node_editor.node_socket.Socket` types from which the `Sockets` will be auto created
        :param outputs: list of :class:`~node_editor.node_socket.Socket` types from which the `Sockets` will be auto created

        :Instance Attributes:

            - **scene** - reference to the :class:`~node_editor.node_scene.Scene`
            - **grNode** - Instance of :class:`~node_editor.node_graphics_node.QDMGraphicsNode` handling graphical representation in the ``QGraphicsScene``. Automatically created in the constructor
            - **content** - Instance of :class:`~node_editor.node_graphics_content.QDMGraphicsContent` which is child of ``QWidget`` representing container for all inner widgets inside of the Node. Automatically created in the constructor
            - **inputs** - list containin Input :class:`~node_editor.node_socket.Socket` instances
            - **outputs** - list containin Output :class:`~node_editor.node_socket.Socket` instances
    ...
    def getNodeContentClass(self):
        """Returns class representing node_editor content"""
    ...
    def onEdgeConnectionChanged(self, new_edge: 'Edge'):
        ...
        :param new_edge: reference to the changed :class:`~node_editor.node_edge.Edge`
    ...
    def onInputChanged(self, socket: 'Socket'):
        ...
        :param socket: reference to the changed :class:`~node_editor.node_socket.Socket`
    ...
    def hasConnectedEdge(self, edge: 'Edge'):
        """Returns ``True`` if edge is connected to any :class:`~node_editor.node_socket.Socket` of this `Node`"""
        for socket in (self.inputs + self.outputs):
    ...
    def getSocketPosition(self, index: int, position: int, num_out_of: int=1) -> '(x, y)':
        """
        Get the relative `x, y` position of a :class:`~node_editor.node_socket.Socket`. This is used for placing
    ...
    def getChildrenNodes(self) -> 'List[Node]':
        ...
        :rtype: List[:class:`~node_editor.node_node.Node`]
        """
    ...
    def getInput(self, index: int=0) -> ['Node', None]:
        ...
        :type index: ``int``
        :return: :class:`~node_editor.node_node.Node` which is connected to the specified `Input` or ``None`` if
    ...
    def getInputWithSocket(self, index: int=0) -> [('Node', 'Socket'), (None, None)]:
        ...
        :param index: Order number of the `Input Socket`
        :type index: ``int``
        :return: Tuple containing :class:`~node_editor.node_node.Node` and :class:`~node_editor.node_socket.Socket` which
    ...
    def getInputs(self, index: int=0) -> 'List[Node]':
        ...
        :param index: Order number of the `Input Socket`
        :type index: ``int``
        :return: all :class:`~node_editor.node_node.Node` instances which are connected to the
